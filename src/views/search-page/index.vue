<template>
    <main class="main">
        <h2>
            “{{ searchValue }}”的搜索结果（{{ sorts[0] && sorts[0].value }}）
        </h2>
        <div class="main-tabs">
            <RadioGroup v-model="selSort" type="button">
                <Radio v-for="(i, j) in sorts" :key="j" :label="i.name">
                    {{ i.name }} {{ i.value }}
                </Radio>
            </RadioGroup>
        </div>
        <div v-if="details.length" class="main-conent">
            <card-details
                v-for="(option, index) of details"
                :key="index"
                :data="option"></card-details>
        </div>
...
export default {
    ...
    computed: {
        ...
        details() {
            let list = []
            // 列表数据或者请求数据没有返回空数组
            if (!this.sorts.length || !this.resData.length) {
                return list
            }
    ...
    methods: {
        init() {
            if (!this.menuData || !Object.keys(this.menuData).length) {
                return false
            }
                ...
                this.getArrangementData(arr).forEach(item => {
                    ...
                    switch (clildren.name) {
                        ...
                        case '组件':
                            ...
                            secondLevel =
                                this.componentsTagType[item.tagTypeIndex].label
...
.main {
  ...
  &-tabs {
    padding-bottom: 16px;
    border-bottom: 1px solid rgba(23, 35, 61, .15);
